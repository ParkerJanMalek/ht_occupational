...
os.chdir("G:/VCS/ht_occupational/ht_occupational")
...
model_part_1_prefit_file = 'output/model_part1.file'


fit1 = cloudpickle.load(open(model_part_1_prefit_file,'rb'))
...
model_part_2_prefit_file = 'output/model_part2.file'
fit2 = cloudpickle.load(open(model_part_2_prefit_file,'rb'))
...
axes = az.plot_forest([fit1.sample(5000),fit2.sample(5000)], var_names=['beta'], figsize=(10,7), model_names=['Detect/Non-detect', 'Air concentration'],
                          hdi_prob=0.95, linewidth = 3, markersize=10)
...
posterior = fit1.sample(draws=5000)  # Returns InferenceData

# STEP 2: Optional - Subset first 1000 draws (to mimic PyMC3 `samples=1000`)
posterior_subset = posterior.sel(draw=slice(0, 999))

# STEP 3: Sample from posterior predictive
ppc_train = pm.sample_posterior_predictive(posterior_subset, model=osha_part_1, random_seed=44749)




#ppc_train = pm.sample_posterior_predictive(fit1.sample(5000), samples= 1000, random_seed=44749, model=osha_part_1)
avg_predictions_train = np.where(ppc_train.posterior_predictive['likelihood'][0].mean(axis=0) >= 0.5, 1, 0)
prob_predictions_train = ppc_train.posterior_predictive['likelihood'][0].mean(axis=0)
...
train_p1_samples = pd.DataFrame(ppc_train.posterior_predictive['likelihood'][0].transpose())
...
posterior = fit2.sample(draws=5000)  # Returns InferenceData

# STEP 2: Optional - Subset first 1000 draws (to mimic PyMC3 `samples=1000`)
posterior_subset = posterior.sel(draw=slice(0, 999))


ppc_pt2_train = pm.sample_posterior_predictive(posterior_subset, model=osha_part_2, random_seed=6584)



#ppc_pt2_train = pm.sample_posterior_predictive(fit2.sample(5000), samples= 1000, random_seed=6584, model=osha_part_2)
avg_predictions_pt2_train = ppc_pt2_train.posterior_predictive['likelihood'][0].mean(axis=0)
...
ppc_full_train_pt2 = pm.sample_posterior_predictive(posterior_subset, model=osha_part_2, random_seed=6596)
...
avg_predictions_pt2_full_train = ppc_full_train_pt2.posterior_predictive['likelihood'][0].mean(axis=0)

# Save probabilistic samples - training set part 2 
train_p2_samples = pd.DataFrame(ppc_full_train_pt2.posterior_predictive['likelihood'][0].transpose())
...
# STEP 2: Optional - Subset first 1000 draws (to mimic PyMC3 `samples=1000`)
posterior_subset_sam = sam.sel(draw=slice(0, 999))


ppc_test = pm.sample_posterior_predictive(posterior_subset_sam, random_seed=13464, model=osha_part_1)
avg_predictions_test = np.where(ppc_test.posterior_predictive['likelihood'][0].mean(axis=0) >= 0.5, 1, 0)
avg_probs_test = ppc_test.posterior_predictive['likelihood'][0].mean(axis=0)

# Save probabilistic samples - test set part 1
test_p1_samples = pd.DataFrame(ppc_test.posterior_predictive['likelihood'][0].transpose())
...
np.random.seed(28373)

posterior=fit2.sample(5000)

# STEP 2: Optional - Subset first 1000 draws (to mimic PyMC3 `samples=1000`)
posterior_subset = sam.sel(draw=slice(0, 999))

ppc_p2_test = pm.sample_posterior_predictive(posterior_subset, random_seed=23094, model=osha_part_2)
avg_predictions_pt2_test = ppc_p2_test.posterior_predictive['likelihood'][0].mean(axis=0)

# Save probabilistic samples - test set part 2
test_p2_samples = pd.DataFrame(ppc_p2_test.posterior_predictive['likelihood'][0].transpose())
...
predicted = pd.Series(avg_predictions_pt2_test)[(test_combined['pred_detected'] == 1) & (test_combined['detected'] == 1)]
...
substance_predictions_test = substance_predictions_table(test_p1_samples.drop(columns=[ 'sector_name', 'subsector_name']), test_p2_samples.drop(columns=['sector_name', 'subsector_name']), min_N = 5, sortbyair=True, printout=False)
substance_predictions_test.to_csv('output/TableS1.csv')
substance_predictions_test = substance_predictions_table(full_p1_samples.drop(columns=[ 'sector_name', 'subsector_name']), full_p2_samples.drop(columns=[ 'sector_name', 'subsector_name']), min_N = 5, sortbyair=True, printout=False)
